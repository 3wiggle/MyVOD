...
function Seek(searchString, liste) { // searches a video
	var Anz = 0;
	if ((searchString == '') || (searchString == '*')) { // show all
		...
		for (var i = 0; i < liste.length; i++) { //do the search
			if (liste[i].textContent.toLowerCase().includes(searchString)) {
				...
				//if (liste[i].offsetTop < (window.innerHeight + scrollTop)) {
				//	liste[i].src = liste[VidNr].dataset.src;
				//	liste[i].classList.remove('lazy');
				//}
...
function RandomFilm(liste) { // chooses randomly one movie 
	if (liste.length > 1)
	...
	//liste[VidNr].src = liste[VidNr].dataset.src;
	//liste[VidNr].classList.remove('lazy');
